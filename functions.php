...
function sakura_scripts()
{
    ...
    // 预加载主要样式文件（始终使用生产模式合并资源）
    if (!function_exists('sakurairo_add_cache_control_header')) {
        function sakurairo_add_cache_control_header() {
            if (!is_user_logged_in()) {
                header('Cache-Control: public, max-age=86400, s-maxage=86400');
            }
        }
    }
    add_action('send_headers', 'sakurairo_add_cache_control_header');
